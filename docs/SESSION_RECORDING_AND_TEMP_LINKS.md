...
| `AuthManager` | `apps/gateway/internal/auth/auth.go:51` | User model with `ID`, `Email`, `Credits`, `StorageUsed`, `StorageQuota` (100 GB). JWT-based access/refresh tokens. No role/claim model beyond `Role: "user"` in DB rows. |
