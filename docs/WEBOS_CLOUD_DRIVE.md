...
- **10 GB cap**: Per-user storage is hard-capped at 10 GB (10,737,418,240 bytes). The cap is enforced server-side on every upload.
...
|-----------|----------|----------|
| Storage manager | `apps/gateway/internal/storage/storage.go:46` | `StorageManager` with `basePath`, in-memory `files` map, `quotaFn` / `usageFn` hooks. |
| Endpoints | `apps/gateway/internal/tunnel/server.go:393-396` | `/storage/list`, `/storage/upload`, `/storage/download`, `/storage/delete`. All wrapped in `storageAuth` which injects `userId` from the authenticated session. |
| Quota enforcement | `apps/gateway/internal/tunnel/server.go:257-260` | `SetQuotaCheck(authMgr.CheckStorageQuota)` and `SetUsageFunc(authMgr.AddStorageUsed)` are wired in at startup. |
| Auth quota | `apps/gateway/internal/auth/auth.go:316-336` | `CheckStorageQuota(userID, additionalBytes)` returns `ErrQuotaExceeded` if `StorageUsed + additionalBytes > StorageQuota`. `AddStorageUsed` increments the counter. Default quota = 10 GB (`10 * 1024 * 1024 * 1024`). |
| Frontend UI | `apps/frontend/app/webos/apps/files/Files.tsx` | Minimal file browser: list, upload, delete, download. Shows usage bar. Hard-codes `quota.total = 10737418240`. |
| Startup comment | `run-backend.sh:283` | Prints `Storage: 10GB per user (local: /tmp/freecompute-storage/)`. |
...
## 4. Per-User Namespace + 10 GB Enforcement
...
1. `StorageHandler.Upload` calls `StorageManager.WriteFile` (`storage.go:108`).
2. `WriteFile` invokes `s.quotaFn(userID, size)` before writing.
3. `quotaFn` is set to `authMgr.CheckStorageQuota` (`server.go:257`), which checks `user.StorageUsed + size > user.StorageQuota` (10 GB) and returns `ErrQuotaExceeded`.
...
**No changes are required to enforce the 10 GB cap** — the hooks are already wired. The only addition needed: the new `RemoteVM` backend should also surface quota errors (e.g., the remote agent returns `507 Insufficient Storage`, which the gateway maps to `ErrQuotaExceeded`).
...
1. **Quota error UX**: Parse `507` responses on upload and show `"Drive full — 10 GB limit reached"`.
...
| 6 | `apps/gateway/internal/tunnel/server.go:163` | Replace `storage.NewStorageManager(logger, "/tmp/freecompute-storage")` with a factory that reads `FREECOMPUTE_STORAGE_BACKEND` and returns the correct `Backend`. |
| 7 | `apps/gateway/internal/tunnel/server.go:393-396` | Add `/storage/mkdir` endpoint (optional, for folder creation). |
| 8 | `run-backend.sh:283` | Update echo to reflect new env-driven backend. |
...
- [ ] Upload until total reaches 10 GB → subsequent upload returns `507 Insufficient Storage` with `{"error":"storage quota exceeded"}`.
...
```bash
echo "Storage: 10GB per user (backend=${FREECOMPUTE_STORAGE_BACKEND:-local}, root=${FREECOMPUTE_STORAGE_ROOT:-/tmp/freecompute-storage})"
```
